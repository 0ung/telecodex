...
Telegram으로 로컬 Codex를 제어하는 작은 도구
...
SmartScreen은 평판 기반 보호 기능이므로, 파일이 악성코드인지 아닌지를 단순히 즉시 단정하는 것이 아니라, 서명 여부와 게시자 정보, 파일의 배포 및 실행 이력 등을 함께 평가합니다.
...
현재 프로젝트는 취미용 배포를 전제로 하고 있으므로,
README에서는 직접 빌드를 우선 권장합니다.
